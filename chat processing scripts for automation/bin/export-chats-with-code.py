...
def extract_code_blocks(text):
    ...
    Returns a list of dictionaries with 'language' and 'content' keys.
    """
    code_blocks = []
    ...
    # Find all code blocks using markdown syntax with backticks
    import re
...
def enrich_requests_with_code_blocks(requests):
    """
    Process requests and extract code blocks from message content.
    Returns enriched requests with codeBlocks array.
    """
    enriched_requests = []
    ...
    for request in requests:
        ...
        # Extract code blocks from the value/message content
        message_content = request.get("value", "")
        if message_content:
            code_blocks = extract_code_blocks(message_content)
            if code_blocks:
                enriched_request["codeBlocks"] = code_blocks
...
def export_chat_to_json(chat, output_dir, sequence_number=None, verbose=False):
    """Export a single chat to a JSON file with code blocks extracted."""
    ...
    # Enrich requests with extracted code blocks
    enriched_requests = enrich_requests_with_code_blocks(chat["requests"])
    ...
    export_data = {
        ...
        "note": "This export includes extracted code blocks in the 'codeBlocks' field of each request"
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Export VS Code Copilot chats with extracted code blocks from a specific project workspace",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Export chats from a specific project (exports to {project}/batch chat exports/)
  python3 export-chats-with-code.py -p "/path/to/project/folder"
  
  # Export with custom output directory
  python3 export-chats-with-code.py -p "/path/to/project/folder" -o ~/Desktop/my_chats
  
  # Export with verbose output
  python3 export-chats-with-code.py -p "/path/to/project" -v
  
  # Show help
  python3 export-chats-with-code.py --help
        """
    )
    ...
    print(f"✓ Code blocks extracted and embedded in 'codeBlocks' field")
